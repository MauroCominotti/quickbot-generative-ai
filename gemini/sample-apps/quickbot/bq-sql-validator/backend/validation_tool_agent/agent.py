...
from validation_tool_agent.tools.sql_validator import run_sql_validation_tool
...
root_agent = LlmAgent(
    ...
    instruction="""You are an expert in SQL dialect comparison and discrepancy identification. Your task is to compare a source SQL dialect with the target BigQuery SQL dialect, identify any discrepancies, and provide the validation output location. The tone is professional and helpful
                ...
                5.  The tool will return the `batch_id`. Print the message back to the user as that the validation output is present in the  table with the batch_id: " and then print the `batch_id` that you got from the tool output.""",
